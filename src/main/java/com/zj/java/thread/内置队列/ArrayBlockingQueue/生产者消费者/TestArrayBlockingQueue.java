...
/**
 * @Author: zhoujun
 * @Date: 2023/2/18 20:20
 */
public class TestArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    /**
     * 1. 首先获取当前队头元素并将其保存到局部变量，
     * 2. 然后重置队头元素为 null，并重新设置队头下标，
     * 3. 递减队列元素计数器，
     * 4. 最后发送信号激活 notFull 的条件队列里面一个因为调用 put 方法而被阻塞的线程。
     */
    private E dequeue() {
        // assert lock.getHoldCount() == 1;
     ...
     * 如果队列已满则阻塞, 当前线程直到队列有空闲并插入成功后返回 true，
     ...
     * 其实如果 count 被声明为 volatile 的这里就不需要加锁了，
     * 因为 volatile 类型的变量保证了内存的可见性，
     * 而 ArrayBlockingQueue 中的 count 并没有被声明为 volatile 的，
     * 这是因为 count 操作都是在获取锁后进行的，而获取锁的语义之一是，
     * 获取锁后访问的变量都是从主内存获取的，这保证了变量的内存可见性。
     * @return 当前队列的尺寸
     */
    public int size() {
        final ReentrantLock lock = this.lock;
    ...
    private class Itr implements Iterator<E> {
        ...
        Itr() {
            ...
            lastRet = NONE;
            final ReentrantLock lock = TestArrayBlockingQueue.this.lock;
            lock.lock();
            try {
                if (count == 0) {
                    // assert itrs == null;
                    ...
                    final int takeIndex = TestArrayBlockingQueue.this.takeIndex;
                    prevTakeIndex = takeIndex;
                    nextItem = itemAt(nextIndex = takeIndex);
                    cursor = incCursor(takeIndex);
                    if (itrs == null) {
                        itrs = new TestArrayBlockingQueue.Itrs(this);
                    } else {
